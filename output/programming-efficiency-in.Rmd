...
`sapply()` is 'simplified' apply that attempts to coerce the result to a multi-dimensional array, *if appropriate*. However, the function is not type consistent and can cause some problems. Below we can see
...
Another efficient way to wrangle data is `tidyr::pivot_longer()`. The function makes "wide" tables "long". 
...
For this example, we will split the data set into a list, where every element of that list is an item. Then, we can loop through the list using `foreach` and the `%dopar%` operator to run a for loop in parallel. We use 25k rows this time. 
...
cfa <- 
  foreach (i = qref, .multicombine = T, .errorhandling = "pass") %dopar% {
    cfa(model =  "skill =~ tc0 + tc1 + tc2 + tc3 + tc4 + tc5 + tc6 + tc7 + tc8 + tc9", data = ValData[ValData$qref == i,], ordered = c('tc0','tc1','tc2','tc3','tc4','tc5','tc6','tc7','tc8','tc9'))}
```
